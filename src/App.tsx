...
export default function App() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      // File selected! Start the fake scanning process
  ...
  const handleGoogleLogin = () => {
    setIsLoggingIn(true);
    setTimeout(() => {
      setUser({ name: 'Pruthvi', isPro: false });
      setIsLoggingIn(false);
      setActiveModal(null);
    }, 800); // 800ms delay to feel real
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4 animate-in fade-in duration-300">
          ...
          {activeModal === 'auth' && (
            <div className="bg-neutral-900/90 border border-white/10 rounded-3xl p-8 w-full max-w-sm flex flex-col items-center gap-4 relative shadow-2xl">
              ...
              <button 
                ...
                className="bg-white text-black w-full rounded-full py-3 text-sm font-medium flex items-center justify-center gap-3 hover:bg-neutral-200 transition-colors disabled:opacity-80"
              >
                {isLoggingIn ? (
                  <><div className="w-4 h-4 border-2 border-black/20 border-t-black rounded-full animate-spin"></div> connecting...</>
                ) : (
                  <>
          ...
          {activeModal === 'pricing' && (
            <div className="bg-neutral-900/90 border border-white/10 rounded-3xl p-8 w-full max-w-3xl relative shadow-2xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-black/50 rounded-2xl p-6 border border-white/10 flex flex-col">
                  <h3 className="text-white text-xl font-medium">basic</h3>
                  <div className="text-white text-3xl font-light mt-2 mb-6">₹0</div>
                  <ul className="text-white/70 text-sm space-y-3 flex-grow">
                ...
                <div className="bg-black/80 rounded-2xl p-6 border border-indigo-500/50 relative overflow-hidden flex flex-col">
                  ...
                  <ul className="text-white/90 text-sm space-y-3 mb-6 flex-grow">
                  ...
                  {!user?.isPro ? (
                    <button 
                      ...
                      className="w-full bg-white text-black font-semibold py-3 rounded-full hover:bg-neutral-200 transition shadow-lg"
                    >
                      upgrade for ₹199
                    </button>
                  ) : (
                    <div className="w-full bg-indigo-500/20 border border-indigo-500/50 text-indigo-300 text-center font-medium py-3 rounded-full">
                      pro activated
                    </div>
                  )}
                </div>
              ...
              {scanState === 'idle' && (
                <div className="text-center animate-in fade-in">
                  ...
                  >
                    ...
                    <span className="text-white text-sm font-medium">click to select your pdf</span>
                  </div>
              ...
              {scanState === 'results' && (
                <div className="animate-in fade-in duration-500 text-left">
                  <div className="flex justify-between items-end mb-6 border-b border-white/10 pb-4">
                    <div>
                      <p className="text-white/50 text-xs font-mono mb-1">file: document_analysis.pdf</p>
          ...
          {activeModal === 'payment' && (
            <div className="bg-neutral-900/90 border border-white/10 rounded-3xl p-8 w-full max-w-sm relative shadow-2xl text-center">
              <button onClick={() => {setActiveModal('pricing');}} className="absolute top-4 left-4 text-white/50 hover:text-white transition text-sm flex items-center gap-1">← back</button>
              <button onClick={() => setActiveModal(null)} className="absolute top-4 right-4 text-white/50 hover:text-white transition text-xl">&times;</button>
